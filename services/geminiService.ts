...
// Definitive fix for Vite projects.
// Vite exposes environment variables to the client-side code via `import.meta.env`.
// The variable MUST be prefixed with VITE_ in the hosting provider (e.g., Render).
function getAiClient(): GoogleGenAI {
   if (!ai) {
       // Access the environment variable using the Vite-specific `import.meta.env`.
